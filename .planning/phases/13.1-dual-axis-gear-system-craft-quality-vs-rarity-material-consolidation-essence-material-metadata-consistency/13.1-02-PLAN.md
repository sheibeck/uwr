...
2. **Fix `tierLabel` to derive from `craftQuality` when present.** The current `tierLabel` computes `'Tier ${qualityTierToNumber(qualityTier)}'`, which always returns "Tier 1" for crafted items (because crafted items always have `qualityTier='common'`). Replace that logic so that when `instance.craftQuality` is defined, the tier number is derived from `craftQuality` instead:
   ```typescript
   function craftQualityToTierNumber(cq: string): number {
     if (cq === 'dented' || cq === 'standard') return 1;
     if (cq === 'reinforced') return 2;
     if (cq === 'exquisite' || cq === 'mastercraft') return 3;
     return 1;
   }
   const tierNumber = instance.craftQuality
     ? craftQualityToTierNumber(instance.craftQuality)
     : qualityTierToNumber(qualityTier);
   const tierLabel = `Tier ${tierNumber}`;
   ```
   This ensures crafted Reinforced items show "Tier 2" and Exquisite/Mastercraft items show "Tier 3", rather than every crafted item showing "Tier 1".

3. **Read `description` from ItemTemplate** and use it when available. Find where the `description` is computed (around lines 144-153). Change the description logic to prefer the template description:
   ```typescript
   const description =
...
4. **Include `craftQuality` in the returned item object.** Find where the item data object is returned/constructed and add `craftQuality` to it. It should be available alongside `qualityTier`, `displayName`, `description`, etc.

**Update `src/App.vue`:**

5. **Display craftQuality in item tooltip with color coding (required).** Find the tooltip rendering section (around line 505-506 where `description` is shown). Show the craft quality level as a colored line in the tooltip for all items that have `craftQuality` defined. Use the following color mapping — this is required, not optional:
   - dented: #888 (gray)
   - standard: #ccc (light gray)
   - reinforced: #6c9 (green)
   - exquisite: #9cf (blue)
   - mastercraft: #f90 (orange)

   Define the mapping inline or as a computed helper:
   ```
   <span v-if="item.craftQuality"
         :style="{ color: craftQualityColor(item.craftQuality), fontSize: '11px', textTransform: 'capitalize' }">
     {{ item.craftQuality }} Quality
   </span>
   ```
   Where `craftQualityColor` maps: `{ dented: '#888', standard: '#ccc', reinforced: '#6c9', exquisite: '#9cf', mastercraft: '#f90' }`.
...
- Check `useInventory.ts` derives `tierLabel` from `craftQuality` when `instance.craftQuality` is defined (dented/standard→Tier 1, reinforced→Tier 2, exquisite/mastercraft→Tier 3)
- Check `useInventory.ts` returns `craftQuality` in item data
- Check `App.vue` renders craftQuality with color coding for all items that have the field (dented=#888, standard=#ccc, reinforced=#6c9, exquisite=#9cf, mastercraft=#f90)
...
- Visually: a crafted Reinforced Longsword shows "Tier 2" in its label, not "Tier 1"
- Visually: a crafted Exquisite Dagger shows "Tier 3" in its label and a blue "exquisite Quality" line in the tooltip
  </verify>
  <done>Frontend reads and displays craftQuality from ItemInstance; tierLabel derives tier number from craftQuality when present (dented/standard=Tier 1, reinforced=Tier 2, exquisite/mastercraft=Tier 3) rather than always showing Tier 1; tooltip shows craft quality level with required color coding (dented=#888, standard=#ccc, reinforced=#6c9, exquisite=#9cf, mastercraft=#f90); template description is read and takes priority over computed description; app compiles without errors</done>
</task>
