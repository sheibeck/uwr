...
2. Handle the `gift-npc` event from LocationGrid. When triggered:
   - Show a simple item selection UI (can reuse existing inventory item list pattern)
   - OR open a browser `prompt()` / custom dialog listing inventory items
   - On item selection, call `window.__db_conn?.reducers.giveGiftToNpc({ characterId, npcId, itemInstanceId })`
   - The simplest approach: add a new small modal/overlay component that lists non-equipped inventory items as clickable rows. On click, call the reducer and close.
...
6. Gift flow consumes item and increases affinity
  </verify>
  <done>
NpcDialogPanel shows affinity tier + progress bar per NPC. Available dialogue options display with unlocked/locked styling. Clicking unlocked option calls chooseDialogueOption reducer. LocationGrid NPC context menu has "Give Gift" option. Gift flow calls giveGiftToNpc with selected inventory item. npcAffinity and npcDialogueOption tables subscribed in useGameData. All reducer calls use object syntax per CLAUDE.md rules.
  </done>
</task>
...
8. Right-click Marla and select "Give Gift" -- should show inventory items
