...
1. **IMPORTANT:** Verify that the existing "Talk" context menu option opens the NPC Dialog panel (Journal tab). When a player clicks "Talk" on an NPC, it should:
   - Call the hailNpc reducer (which logs "You begin to talk with <NPC Name>" to Log panel)
   - Open the NPC Dialog panel (Journal) automatically so the player can see the greeting and dialogue options

2. Add "Give Gift" option to the NPC context menu (below existing "Talk" and "Open Store" options):
   ```typescript
   items.push({
     label: 'Give Gift',
...
3. Add the new emit declaration:
   ```typescript
...
2. Right-click on Marla the Guide and select "Talk"
   - **Log panel** should show: "You begin to talk with Marla the Guide."
   - **Journal panel (NPC Dialog)** should open automatically and show the greeting
   - **Journal** should display the full greeting text (e.g., "Marla the Guide says, '...'")
3. Verify the NPC Dialog panel shows Marla with "Stranger" affinity tier badge
4. Verify dialogue options appear: "Tell me about this area" and "Which way should I go?" should be available
5. Verify "How did you become a guide?" (requires 25 affinity) appears locked/dimmed with requirement text
6. Click an available dialogue option
   - **Journal** should show both player text AND NPC response (NOT in Log)
   - **Log** should NOT show the dialogue content
7. Verify affinity changed (small increase shown in progress bar)
8. Repeat greeting/dialogue until affinity crosses a tier threshold (e.g., 0 → 25)
   - **Log panel** should show: "Your relationship with Marla the Guide improved to Acquaintance."
9. Right-click Marla and select "Give Gift" -- should show inventory items in overlay
10. Select an item to gift
    - Item should be consumed from inventory
    - **Journal** should show gift message and NPC reaction
    - **Log** should show tier change if affinity crossed threshold
11. Repeat greeting after affinity reaches 25+ -- greeting text should change dynamically
12. Verify dialogue options unlock as affinity increases (new options appear when requirements met)
13. Test with Elder Soren and Quartermaster Jyn to confirm all NPCs work

**Logging verification:**
- ✅ All NPC dialogue (greetings, player text, NPC responses, gift reactions) appears in **Journal panel only**
- ✅ "You begin to talk with <NPC Name>" appears in **Log panel only** (when Talk clicked)
- ✅ "Your relationship with <NPC Name> improved to <Tier>" appears in **Log panel only** (when tier changes)
- ❌ Dialogue content should NEVER appear in Log panel
  </how-to-verify>
