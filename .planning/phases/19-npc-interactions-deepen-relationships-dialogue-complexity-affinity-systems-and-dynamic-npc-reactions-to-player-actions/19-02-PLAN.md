...
export const registerNpcInteractionReducers = (deps: any) => {
  ...
  spacetimedb.reducer('choose_dialogue_option', {
    ...
    // Log the player's dialogue choice
  ...
  spacetimedb.reducer('give_gift_to_npc', {
    ...
    // NPC reaction based on current affinity
...
Note: Import `SenderError` from 'spacetimedb/server' at the top. Match the existing reducer registration pattern used in commands.ts -- check how deps are passed and follow the same pattern.
