...
must_haves:
  ...
  artifacts:
    - path: "spacetimedb/src/procedures/llm.ts"
      provides: "generate_content procedure with circuit breaker, fallback, and Anthropic API integration"
      min_lines: 80
    - path: "spacetimedb/src/index.ts"
      provides: "Updated to import procedure file for registration"
      contains: "procedures/llm"
  key_links:
    - from: "spacetimedb/src/procedures/llm.ts"
      to: "spacetimedb/src/index.ts"
      via: "imports spacetimedb from index.ts to register procedure"
      pattern: "import.*spacetimedb.*from"
    - from: "spacetimedb/src/procedures/llm.ts"
...
Purpose: This is the heart of the LLM architecture. The procedure handles the full lifecycle: read config, check circuit breaker, call API, parse response, write result or fallback. It must guarantee that no content row ever stays in 'pending' status.

Output: New procedures directory with llm.ts containing generate_content procedure. Updated index.ts to import and register it.
</objective>
...
<task type="auto">
  <name>Task 1: Create generate_content procedure with circuit breaker and fallback logic</name>
...
This file imports `spacetimedb` from the main schema (`../index`) and defines the `generate_content` procedure. It also imports helpers from `../data/llm_content`.
...
**Helper functions (define before the procedure, in the same file):**

**`writeFallback`**: Picks a random fallback from FALLBACK_CONTENT for the contentType, writes to the appropriate table:
```typescript
function writeFallback(ctx: any, contentType: string, contentId: bigint, reason: string): void {
  ...
  // Deterministic selection: use contentId modulo array length (no Math.random — reducers must be deterministic, but procedures CAN use non-deterministic code; however use contentId for reproducibility)
...
**`writeSuccess`**: Writes generated text with status 'ready':
```typescript
...
- Import ScheduleAt from 'spacetimedb' and TimeDuration from 'spacetimedb'
- Import `t` from 'spacetimedb/server'
  </action>
  <verify>Run `npx tsc --noEmit --project spacetimedb/tsconfig.json` to confirm no type errors. Verify the file contains: `spacetimedb.procedure`, `ctx.http.fetch`, `ctx.withTx`, `writeFallback`, `writeSuccess`, `recordFailure`, `recordSuccess`. Verify no usage of `ctx.db` (only `tx.db` inside withTx callbacks).</verify>
  <done>generate_content procedure defined with full circuit breaker logic, fallback writing, Anthropic API call with 30s timeout, input sanitization, and prompt caching. No content row ever stays in 'pending' status.</done>
</task>

<task type="auto">
  <name>Task 2: Register procedure in index.ts, publish module, and regenerate bindings</name>
  <files>spacetimedb/src/index.ts</files>
  <action>
1. **Add import at top of `spacetimedb/src/index.ts`** (after other imports, before table definitions):
```typescript
import './procedures/llm';
```

This side-effect import ensures the procedure file is loaded and the `spacetimedb.procedure(...)` call executes during module initialization. The procedure file imports `spacetimedb` from `./index` (the schema object), which is fine because the schema is defined before the import resolves at runtime.

**IMPORTANT:** Place this import AFTER the `export const spacetimedb = schema(...)` line to avoid circular dependency issues. The procedure file imports `spacetimedb` from `../index`, so the schema must be exported before the procedure file loads.

Actually, since the procedure file does `import { spacetimedb } from '../index'`, and index.ts defines `spacetimedb` via `schema()`, the import must be placed AFTER the schema export. Add it right after the `export const spacetimedb = schema(...)` line:
...
// Register procedures (must be after schema export)
import './procedures/llm';
```

Wait — top-level imports in ES modules are hoisted. Instead, use dynamic import or restructure. The codebase already handles this pattern: `schema.ts` is separate from `index.ts` in the CLAUDE.md guidance, but this codebase has everything in `index.ts`.

**Revised approach:** Since the procedure file needs the `spacetimedb` object from index.ts, and all imports are hoisted, we need to ensure no circular dependency. Check if the existing pattern (reducers import from index.ts) works — actually, reducers use the `deps` pattern and don't import `spacetimedb` directly. The procedure MUST import `spacetimedb` directly to call `spacetimedb.procedure()`.

**Solution:** Move the procedure import to be a side-effect import placed after the schema export. In practice with ES module hoisting, this requires using a workaround. The cleanest approach: make the procedure file NOT import spacetimedb from index. Instead, pass it via a registration function similar to reducers:

Create a `registerLlmProcedures` function in `spacetimedb/src/procedures/llm.ts` that takes `{ spacetimedb, ... }` as deps — same pattern as reducers. Then call it from index.ts after schema export.

**Updated plan for procedures/llm.ts:** Restructure to export `registerLlmProcedures(deps)` instead of importing spacetimedb at top level. The deps object should include: `spacetimedb, ScheduleAt, TimeDuration` (t is imported from spacetimedb/server directly in the procedure file).

**Updated plan for index.ts:**
- Import at top: `import { registerLlmProcedures } from './procedures/llm';`
- After `registerReducers(reducerDeps);`, add:
```typescript
...
4. **Verify bindings generated** — check that `src/module_bindings/` contains references to `generateContent` procedure and the new tables (`generatedQuestText`, `generatedEventText`, `llmConfig`).
  </action>
  <verify>Run `spacetime logs uwr` to check for startup errors. Verify `src/module_bindings/` contains generateContent procedure type. Run `npx tsc --noEmit` (frontend) to confirm no client type errors after binding regeneration.</verify>
  <done>Procedure registered in module, module published to SpacetimeDB, client bindings regenerated with generate_content procedure type available for client consumption in Plan 03.</done>
</task>
...
- Client bindings include generateContent procedure
...
<success_criteria>
generate_content procedure is deployed and callable. Client bindings include the procedure type. The full failure handling pipeline works: missing config serves fallback, open circuit serves fallback, API error records failure and serves fallback, 3 failures open circuit, circuit auto-resets after 5 minutes. Successful API calls write 'ready' status text.
</success_criteria>
