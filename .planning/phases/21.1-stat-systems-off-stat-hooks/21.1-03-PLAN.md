...
Output: Salvage drops recipe scrolls with INT-boosted probability; buy/sell reducers apply CHA-derived vendor modifiers; faction and affinity gains multiplied by CHA offset.
</objective>
...
<task type="auto">
  <name>Task 1: INT salvage scroll drop + CHA vendor formula</name>
  <files>spacetimedb/src/reducers/items.ts, spacetimedb/src/helpers/character.ts</files>
  <action>
    **Part A: Replace salvage auto-learn with INT-boosted scroll drop in reducers/items.ts**
    ...
    //   import { findItemTemplateByName } from '../helpers/items.js';
    ...
    **Part B: Update vendorBuyMod/vendorSellMod formula in helpers/character.ts**
    ...
    **Part C: Apply vendorBuyMod and vendorSellMod in buy_item and sell_item in reducers/items.ts**
  ...
  <verify>
    ...
    3. Grep: `grep -n "RecipeDiscovered.insert" C:/projects/uwr/spacetimedb/src/reducers/items.ts` — should show ZERO occurrences in salvage_item (auto-learn removed). learn_recipe_scroll may still have it.
    4. Grep: `grep -n "CHA_VENDOR_SCALE\|vendorBuyModRaw" C:/projects/uwr/spacetimedb/src/helpers/character.ts` — new formula present.
    5. Grep: `grep -n "vendorBuyMod\|vendorSellMod" C:/projects/uwr/spacetimedb/src/reducers/items.ts` — two occurrences (buy_item and sell_item).
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: CHA faction and affinity hooks</name>
