...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/composables/useGameData.ts"
      provides: "useTable subscriptions for worldEvent (as worldEventRows), eventContribution, eventSpawnEnemy, eventSpawnItem, eventObjective"
      contains: "worldEventRows"
    - path: "src/App.vue"
    ...
    - path: "src/components/ActionBar.vue"
      provides: "Events button with active-event badge indicator, 'worldEvents' in PanelKey union"
  ...
  key_links:
    ...
    - from: "src/App.vue"
      ...
      via: "worldEventRows destructured from useGameData for banner watch and panel props"
      pattern: "worldEventRows"
...
**CRITICAL NAMING: The existing useGameData.ts already exports `worldEvents` bound to `tables.eventWorld` (the log table). To avoid a naming collision, name the new WorldEvent subscription `worldEventRows`.**
...
Add all 5 to the composable's return object with the key `worldEventRows` (NOT `worldEvents` which is already taken). Also add to the subscription SQL list:
```
...
- `worldEventRows` — array of WorldEvent rows
...
Computed properties:
- `activeEvents` — worldEventRows filtered by status === 'active'
- `historyEvents` — worldEventRows filtered by status !== 'active', sorted by resolvedAt desc
...
5. useGameData.ts exports worldEventRows (NOT worldEvents — that name is already used for eventWorld log table), eventContributions, eventSpawnEnemies, eventSpawnItems, eventObjectives
6. styles.ts exports worldEventBanner style
7. No duplicate `worldEvents` variable in useGameData.ts
  </verify>
  <done>
WorldEventPanel component exists with Active + History tabs. Active shows event name, region, objective progress, contribution tier (Bronze/Silver/Gold), and rewards preview. History shows event name, outcome (color-coded), consequence, and timestamp. useGameData subscriptions wired for all 5 event tables with worldEventRows key (avoiding naming collision with existing worldEvents). Banner style defined.
  </done>
</task>
...
- **Use `worldEventRows` (NOT `worldEvents`) from useGameData — `worldEvents` is the existing eventWorld log table**

**In `src/components/ActionBar.vue`:**

1. **Add 'worldEvents' to the PanelKey union type** (currently at ~line 77):
```typescript
type PanelKey =
  | 'character'
  | 'characterInfo'
...
2. Add `hasActiveEvents` to the component's props (Boolean, passed from App.vue):
```typescript
...
3. Add the button in the template near the Renown button:
```html
<button
...
2. **Destructure** `worldEventRows`, `eventContributions`, `eventObjectives` from useGameData (add to existing destructuring). **NOT `worldEvents`** — that is the existing eventWorld log table variable.

3. **Register 'worldEvents' in usePanelManager** — add to the defaults object (currently at ~line 1879):
```typescript
} = usePanelManager({
  // ... existing entries ...
  worldEvents: { x: 600, y: 140 },   // <-- ADD THIS
}, {
```

4. **Computed** hasActiveEvents:
```typescript
const hasActiveEvents = computed(() => (worldEventRows.value as any[])?.some((e: any) => e.status === 'active') ?? false);
```

5. **Banner overlay** — Add a watch on worldEventRows for new active events:
```typescript
...
7. **Panel rendering** — Add WorldEventPanel div following the exact pattern of other panels. Use `panels.worldEvents.open` for visibility (matching the usePanelManager pattern):
```html
...
```html
<ActionBar ... :has-active-events="hasActiveEvents" />
```
  </action>
  <verify>
1. `npx tsc --noEmit` passes (client compiles)
2. ActionBar has "Events" button with yellow badge dot when active events exist
3. ActionBar PanelKey type includes 'worldEvents'
4. App.vue usePanelManager defaults include `worldEvents: { x: 600, y: 140 }`
5. App.vue uses `panels.worldEvents.open` for v-if on WorldEventPanel div
6. App.vue has banner overlay that appears on new world events and auto-dismisses after ~5 seconds
7. WorldEventPanel receives all required props from App.vue using worldEventRows (not worldEvents)
8. Panel shows/hides when Events button is clicked via togglePanel('worldEvents')
9. hasActiveEvents computed correctly filters worldEventRows by status === 'active'
  </verify>
  <done>
WorldEventPanel fully wired into App.vue with action bar toggle button and active-event badge. 'worldEvents' added to PanelKey union and usePanelManager defaults. Panel visibility uses panels.worldEvents.open pattern. Banner overlay fires on new world events (watched via worldEventRows), auto-dismisses after 5 seconds. All props passed from useGameData subscriptions using worldEventRows key. Panel shows/hides on button click. Client compiles.
  </done>
</task>
...
5. ActionBar PanelKey includes 'worldEvents'
6. Banner overlay fires on new world events, auto-dismisses after ~5 seconds
7. useGameData.ts exports subscriptions for all 5 event tables with worldEventRows key (no naming collision)
8. App.vue wires WorldEventPanel with all required props using worldEventRows
9. usePanelManager includes worldEvents default position
10. panels.worldEvents.open controls panel visibility
11. Client TypeScript compiles without errors
12. Human verifies: event fire, banner, contribution, resolve, tiered rewards, history display
</verification>
