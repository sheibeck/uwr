...
must_haves:
  truths:
    ...
    - "WorldStatTracker table stores server-wide stat counters (statKey, currentValue, fireThreshold, eventKeyToFire) for threshold-triggered event firing (REQ-032)"
    - "World event data constants define at least 2 events with tier thresholds, success/failure rewards, consequence types, and consequenceTextStub"
    ...
    - "incrementWorldStat helper increments a stat counter and auto-fires the associated world event when threshold is crossed"
    - "fireWorldEvent writes consequenceTextStub from event definition to the WorldEvent row at insert time"
  artifacts:
    - path: "spacetimedb/src/schema/tables.ts"
      provides: "WorldEvent, EventContribution, EventSpawnEnemy, EventSpawnItem, EventObjective, WorldStatTracker table definitions"
    ...
    - path: "spacetimedb/src/helpers/world_events.ts"
      provides: "fireWorldEvent, resolveWorldEvent, applyConsequence, awardEventRewards, spawnEventContent, incrementWorldStat helpers"
...
<objective>
Build the backend foundation for the World Events System: 7 new database tables (WorldEvent, EventContribution, EventSpawnEnemy, EventSpawnItem, EventObjective, EventDespawnTick, WorldStatTracker), event data constants with Bronze/Silver/Gold tier reward specs and success/failure consequence types, and helper functions for the full event lifecycle (fire, resolve, reward, consequence, spawn, despawn scheduling, threshold-triggered stat tracking).

Purpose: Provide the data layer and business logic that reducers and combat/travel hooks will wire into in Plan 02. WorldStatTracker + incrementWorldStat satisfy REQ-032 (threshold-triggered world events).
...
Key existing files:
@spacetimedb/src/schema/tables.ts — Add 6 new tables, include in schema() export
...
<task type="auto">
  <name>Task 1: Define 7 new tables and create event data constants</name>
  <files>
    spacetimedb/src/schema/tables.ts
    spacetimedb/src/schema/scheduled_tables.ts
    spacetimedb/src/data/world_event_data.ts
...
Add 6 new tables BEFORE the `schema()` export call. Follow existing table definition patterns.
...
6. **WorldStatTracker** table — server-wide stat counters for threshold-triggered event firing (REQ-032):
```typescript
...
Add ALL 6 tables (WorldEvent, EventContribution, EventSpawnEnemy, EventSpawnItem, EventObjective, WorldStatTracker) to the `schema()` export call at the bottom of tables.ts.
...
```
  ...
  <verify>
Run `npx tsc --noEmit --project spacetimedb/tsconfig.json` — no type errors in tables.ts, scheduled_tables.ts, or world_event_data.ts. Confirm all 6 tables + EventDespawnTick appear in their respective `schema()` calls. Confirm WorldEvent table has BOTH successConsequenceType AND failureConsequenceType columns. Confirm WorldStatTracker table has statKey (unique), currentValue, fireThreshold, eventKeyToFire, fired columns. Confirm NO RegionAdjacency table exists anywhere. Confirm both event definitions include consequenceTextStub.
  </verify>
  <done>
7 new tables defined (WorldEvent with dual consequence types, EventContribution with count for tier tracking, EventSpawnEnemy, EventSpawnItem, EventObjective, WorldStatTracker for REQ-032 threshold-triggered events, EventDespawnTick scheduled). Event data constants file exists with types (including consequenceTextStub on WorldEventDefinition), admin guard, and 2 event definitions (time-based and threshold-race) with Bronze/Silver/Gold tier reward specs, both success/failure consequences, and consequenceTextStub values. No RegionAdjacency table.
  </done>
</task>
...
1. **fireWorldEvent(ctx, eventKey, deps)** — Look up eventKey in WORLD_EVENT_DEFINITIONS. Resolve regionKey to regionId via `ctx.db.region.iter()` name match. Check one-time event guard (reject if same eventKey already exists with status 'success' or 'failed' and isRecurring is false). Insert WorldEvent row — **CRITICAL (REQ-034): write `eventDef.consequenceTextStub ?? ''` to the WorldEvent row's `consequenceText` field at insert time**. Call spawnEventContent. Append world event log via appendWorldEvent. Return inserted row.
...
7. **incrementWorldStat(ctx, statKey, amount, deps)** — (REQ-032: threshold-triggered world events) Look up WorldStatTracker row by statKey via `ctx.db.worldStatTracker.by_stat_key.filter(statKey)`. If no row exists, return (stat not configured). If `fired` is already true, return (already triggered). Increment `currentValue` by `amount`. If `currentValue >= fireThreshold`, set `fired = true`, update the row, then call `fireWorldEvent(ctx, row.eventKeyToFire, deps)`. Otherwise just update the row with the new currentValue. This helper is called from combat kill hooks and quest completion hooks in Plan 02.
...
Use `any` typed ctx and deps parameters following the existing helper pattern in this codebase.
  ...
  <verify>
Run `npx tsc --noEmit --project spacetimedb/tsconfig.json` — confirm no type errors. Verify world_events.ts exports all 7 functions: fireWorldEvent, spawnEventContent, resolveWorldEvent, applyConsequence, awardEventRewards, checkTimeBasedExpiry, incrementWorldStat. Verify zero-contribution guard exists in awardEventRewards (if count === 0n continue). Verify applyConsequence reads from both successConsequenceType and failureConsequenceType based on outcome parameter. Verify fireWorldEvent writes consequenceTextStub to WorldEvent.consequenceText at insert time. Verify incrementWorldStat checks threshold crossing and calls fireWorldEvent.
  </verify>
  <done>
World event helper module exists with 7 exported functions. resolveWorldEvent handles both success and failure paths. awardEventRewards implements Bronze/Silver/Gold tiers with fixed per-tier rewards and zero-contribution guard. applyConsequence switches on consequence type for both outcomes. EventDespawnTick scheduled at 2-minute post-resolve. One-time event guard prevents re-firing. fireWorldEvent writes consequenceTextStub to WorldEvent.consequenceText at insert time (REQ-034). incrementWorldStat checks threshold crossing and auto-fires events (REQ-032). All imports reference existing helpers — no duplicated logic.
  </done>
</task>
...
2. tables.ts contains WorldEvent (with successConsequenceType AND failureConsequenceType), EventContribution, EventSpawnEnemy, EventSpawnItem, EventObjective, WorldStatTracker — NO RegionAdjacency
3. scheduled_tables.ts contains EventDespawnTick with scheduled: 'despawn_event_content'
4. All 7 tables are included in their respective schema() export calls (6 in tables.ts, 1 in scheduled_tables.ts)
5. world_event_data.ts exists with WORLD_EVENT_DEFINITIONS (2+ events with consequenceTextStub), ADMIN_IDENTITIES, types
6. world_events.ts exists with 7 exported lifecycle functions (including incrementWorldStat)
7. awardEventRewards skips count === 0n participants (zero contribution = no reward)
8. Each event definition has both successConsequenceType, failureConsequenceType, and consequenceTextStub
...
11. WorldStatTracker table has statKey (unique), currentValue, fireThreshold, eventKeyToFire, fired columns
12. incrementWorldStat helper checks threshold crossing and calls fireWorldEvent (REQ-032)
13. fireWorldEvent writes consequenceTextStub to WorldEvent.consequenceText at insert time (REQ-034)
</verification>

<success_criteria>
Backend foundation complete: 7 new tables defined (WorldEvent with dual consequences, EventContribution for tiered tracking, EventSpawnEnemy/Item for self-contained event content, EventObjective, WorldStatTracker for threshold-triggered events, EventDespawnTick), event data constants with 2 event definitions using Bronze/Silver/Gold tiers and consequenceTextStub, and helper functions covering the full event lifecycle including incrementWorldStat. TypeScript compiles without errors. Ready for Plan 02 to wire reducers and hooks.
</success_criteria>
