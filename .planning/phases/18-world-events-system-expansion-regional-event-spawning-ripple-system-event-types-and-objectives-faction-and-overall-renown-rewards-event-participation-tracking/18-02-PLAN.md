...
must_haves:
  truths:
    - "Admin can fire a world event via fire_world_event reducer"
  ...
  artifacts:
    - path: "spacetimedb/src/reducers/world_events.ts"
      provides: "fire_world_event, join_world_event, resolve_world_event reducers"
...
<task type="auto">
  <name>Task 1: Create world event reducers and register them</name>
  <files>
    spacetimedb/src/reducers/world_events.ts
    spacetimedb/src/reducers/index.ts
  </files>
...
import { EVENT_STATUS } from '../data/world_event_data';

export function registerWorldEventReducers(deps: any) {
  const { spacetimedb } = deps;

  // REQ-031: Admin-triggered world events
  // Using the grant_test_renown pattern (no admin guard) for initial implementation.
  // Add proper admin identity check as a hardening follow-up.
...
Place both after the existing `registerRenownReducers` line.
  ...
  <verify>
Run `npx tsc --noEmit --project spacetimedb/tsconfig.json` — no errors. Confirm `world_events.ts` exports `registerWorldEventReducers`. Confirm `index.ts` imports and calls it.
  </verify>
  <done>
3 world event reducers registered (fire_world_event, join_world_event, resolve_world_event). Reducer registration follows established pattern. Admin fire uses region name resolution (not hardcoded IDs). Join validates ownership and prevents duplicate participation.
  </done>
</task>
...
5. `spacetime logs uwr` shows seeding ran without errors
6. Client TypeScript compiles
</verification>

<success_criteria>
World event system fully functional on the backend: events can be fired (admin), joined (players), contributed to (via combat kills in event regions), and resolved (auto via objective or manual). Stat trackers increment on combat/quest completion and auto-fire events at thresholds. Ripple cascading works. Module published and bindings generated. Ready for Plan 03 client UI.
</success_criteria>
