...
8. **Add a `filteredClassOptions` computed** — Returns class names allowed by the selected race:
   ```typescript
...
Run `npx tsc --noEmit` (or check if Vite dev server shows no errors).
  ...
  <done>
useGameData returns `races` from `useTable(tables.race)`. useCharacterCreation uses `raceId` instead of `race`, sends `BigInt(raceId)` to the reducer, exposes `selectedRaceRow` and `filteredClassOptions` computeds.
  </done>
</task>
...
1. **Pass races to CharacterPanel** — Find the `<CharacterPanel>` usage at line 125-141. Add the `races` prop. The `races` data comes from `useGameData()` which is already destructured in App.vue. Find where `useGameData` is destructured and add `races` to the destructured list if not already there. Then pass to CharacterPanel:
   ```html
   <CharacterPanel
     ...existing props...
     :races="races"
   ...
   />
   ```
   The `selectedRaceRow` and `filteredClassOptions` come from `useCharacterCreation` — find where its return value is destructured (around line 738) and add them.

2. **Update the update:newCharacter event handler** — At line 134, the current handler is:
   ```html
...
4. **Add race info display** — Below the race dropdown, add a div showing the selected race's description and stat bonuses:
   ...
   <div v-if="selectedRaceRow" :style="styles.roster">
     ...
     <div v-if="selectedRaceRow.availableClasses !== 'all'" :style="styles.subtle">
   ...
   const onRaceChange = (event: Event) => {
     ...
     if (updated.className && props.filteredClassOptions) {
       ...
       if (newRace) {
         ...
         if (allowed && allowed.trim().toLowerCase() !== 'all') {
...
9. Stat bonuses: Switch to Stats panel — verify STR is higher than a normal Warrior would have (Ironclad gives +2 STR)
