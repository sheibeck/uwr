...
1. **Destructure new deps** — At line 1-25, add `isClassAllowed` and `RACE_DATA` (not strictly needed but useful for reference) to the destructured `deps` object. `isClassAllowed` is already in `reducerDeps` (line 5692 of index.ts).
...
5. `create_character` reducer accepts `raceId: t.u64()` (not `race: t.string()`)
6. Reducer validates race exists, is unlocked, and class is allowed
7. Racial stat bonuses are added to computeBaseStats output before computing derived stats
8. Character row stores `race: raceRow.name` (display name from the Race row)
9. Module publishes without errors
10. Generated bindings include `tables.race` and updated reducer type
</verification>
