...
**E. Add `ensureFoodTemplates` function in `index.ts` (near `ensureStarterItemTemplates` at line 3128):**
...
**F. Add `ensureHungerDecayScheduled` function (near `ensureHealthRegenScheduled` at line 4244):**
...
**G. Create `spacetimedb/src/reducers/hunger.ts`:**
...
**K. Update `delete_character` in `spacetimedb/src/reducers/characters.ts`:**
...
**L. Create `spacetimedb/src/views/hunger.ts`:**
...
**M. Add `Hunger` to ViewDeps in `spacetimedb/src/views/types.ts`:**

Add `Hunger: any;` to the ViewDeps type.

**N. Register hunger views in `spacetimedb/src/views/index.ts`:**

Import `registerHungerViews` from `./hunger` and call it in `registerViews`.

**O. Add `Hunger` to the viewDeps object in `index.ts`** (search for where viewDeps is constructed and registerViews is called — it should be near reducerDeps).
  </action>
  <verify>
