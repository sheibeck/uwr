...
Also call `ensureGearRecipeTemplates(ctx)` after material templates are created and after gear item templates exist.
  ...
  <verify>
    - Check seeding functions exist: grep for `ensureGearMaterialItemTemplates` and `ensureGearRecipeTemplates` in ensure_items.ts and ensure_content.ts
    - Check LootTableEntry additions in ensure_enemies.ts: grep for material names (rough_hide, bone_shard, etc.)
  ...
  <done>
    ...
    - Seeding order is correct (materials before nodes/loot)
  </done>
</task>
