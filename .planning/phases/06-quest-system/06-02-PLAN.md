...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/App.vue"
      provides: "Quest data wiring, accept/complete reducer calls, computed filters, quests panel block"
      contains: "acceptFactionQuest"
    - path: "src/components/ActionBar.vue"
      provides: "Quests button in action bar"
      contains: "quests"
  key_links:
    ...
    - from: "src/components/ActionBar.vue"
      to: "src/App.vue"
      via: "emit toggle quests panel"
      pattern: "toggle.*quests"
    - from: "src/App.vue"
      to: "src/components/QuestPanel.vue"
      via: "panels.quests floating panel block renders QuestPanel"
      pattern: "panels\\.quests.*QuestPanel"
---

<objective>
Frontend quest panel UI: create a full three-tab panel (Available, Active, Log) showing faction-gated quests, accept/complete actions, and quest progress. Wire new table subscriptions and reducer calls in App.vue. Register the quests panel in the panel manager, add a Quests button to ActionBar, and add the floating panel block in App.vue template.

Purpose: Give players the ability to browse, accept, track, and complete faction quests through the UI.
Output: Working quest panel with all three tabs, reducer calls wired, data flowing from server subscriptions, panel accessible via ActionBar button.
</objective>
...
<task type="auto">
  <name>Task 1: Add table subscriptions, wire quest data, and register quests panel in App.vue</name>
  <files>
    src/composables/useGameData.ts
    src/components/ActionBar.vue
    src/App.vue
...
After the existing `const [questInstances] = useTable(tables.questInstance);` line, add:
```typescript
const [factionQuests] = useTable(tables.factionQuest);
const [playerFactionQuests] = useTable(tables.playerFactionQuest);
```

Add `factionQuests` and `playerFactionQuests` to the return object (near the existing `questInstances` and `questTemplates` returns).

**2. Add `'quests'` to the PanelKey union type in `src/components/ActionBar.vue`:**

In the `type PanelKey` union (currently includes 'character', 'inventory', 'hotbar', 'friends', 'group', 'log', 'stats', 'crafting', 'journal', 'renown', 'loot', 'travel', 'travelPanel', 'combat', 'help'), add `| 'quests'` to the union.

**3. Add a "Quests" button to the ActionBar template in `src/components/ActionBar.vue`:**

Inside the `<template v-if="hasActiveCharacter">` block, after the Renown button and before the Travel button, add:
```html
<button
...
**4. Wire quest data and reducers in `src/App.vue`:**

In the `import QuestPanel` line area (alongside the other component imports like `import RenownPanel from './components/RenownPanel.vue'`), ensure QuestPanel is imported:
```typescript
import QuestPanel from './components/QuestPanel.vue';
```
(QuestPanel.vue may already be imported if it existed previously. If so, no change needed.)

In the destructuring of `useGameData()` (the large destructuring block ending with `} = useGameData();`), add `factionQuests` and `playerFactionQuests` to the destructured list.

**5. Add computed properties in `src/App.vue`:**

After the existing `characterQuests` computed property (which filters `questInstances` by `selectedCharacter`), add:
...
**6. Add useReducer calls in `src/App.vue`:**

Near the other `useReducer` declarations (after the existing block that includes `hailNpcReducer`, `giveGiftReducer`, etc.), add:
...
**7. Register 'quests' in the panel manager defaults in `src/App.vue`:**

In the `usePanelManager({...})` call (the large object with keys like `group`, `travel`, `hotbar`, `character`, `inventory`, etc.), add a `quests` entry alongside the other panels:
```typescript
quests: { x: 600, y: 140 },
```

**8. Add the Quests floating panel block in the `src/App.vue` template:**

After the Renown Panel block (the `<!-- Renown Panel -->` div that uses `panels.renown`) and before the Loot Panel block (`<!-- Loot Panel -->`), add a new Quests Panel block following the exact same pattern as the Renown Panel:

```html
<!-- Quests Panel -->
<div v-if="panels.quests && panels.quests.open" data-panel-id="quests" :style="{ ...styles.floatingPanel, ...(panelStyle('quests').value || {}) }" @mousedown="bringToFront('quests')">
  <div :style="styles.floatingPanelHeader" @mousedown="startDrag('quests', $event)"><div>Quests</div><button type="button" :style="styles.panelClose" @click="closePanelById('quests')">x</button></div>
  <div :style="styles.floatingPanelBody"><QuestPanel
    :styles="styles"
...
```
  ...
  <verify>
TypeScript check: `cd C:/projects/uwr && npx vue-tsc --noEmit` should produce no errors related to QuestPanel, factionQuests, or playerFactionQuests (may have pre-existing unrelated warnings). Verify that ActionBar.vue includes 'quests' in PanelKey and has the Quests button. Verify App.vue has the `panels.quests` floating panel block.
  </verify>
  <done>
useGameData exports factionQuests and playerFactionQuests subscriptions. App.vue has computed characterFactionQuests, reducer handlers for accept/complete, passes all necessary data as props to QuestPanel, has 'quests' registered in the panel manager defaults, and has the floating panel block rendering QuestPanel. ActionBar.vue has 'quests' in PanelKey and a Quests toggle button.
  </done>
</task>
...
<verification>
1. Quest panel opens from ActionBar toggle button
