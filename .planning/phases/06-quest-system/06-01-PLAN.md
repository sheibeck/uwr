...
4. In `spacetimedb/src/reducers/combat.ts`, add a kill_loot drop check in the kill resolution section. Find where `updateQuestProgressForKill` is called (around line 331+). After that call, add a new function `rollKillLootDrop` and call it:

```typescript
const rollKillLootDrop = (
  ctx: any,
  character: any,
...
5. In `spacetimedb/src/index.ts`, ensure `startCombatForSpawn` and `spawnEnemyWithTemplate` are passed through the deps object to `registerReducers`. Check if they're already in deps; if not, add them.
  ...
  <verify>Run `npx tsc --noEmit --project spacetimedb/tsconfig.json` — should compile with no errors. Verify that `registerQuestReducers` is imported and called in reducers/index.ts.</verify>
  <done>All four new quest types have backend logic: kill_loot drops from combat kills, explore completes via loot_quest_item reducer, delivery auto-completes in hailNpc (without blocking normal dialogue), boss_kill uses existing kill tracking. Named enemy pull spawns combat. Aggro chance on quest item loot works.</done>
</task>
...
- combat.ts rollKillLootDrop is called after updateQuestProgressForKill
</verification>
...
- Kill loot drops wired into combat kill loop
