...
must_haves:
  truths:
    ...
    - "Quest item nodes appear in location panel with loot interaction behind a client-side cast timer"
  ...
  key_links:
    ...
    - from: "src/components/LocationGrid.vue"
      to: "src/App.vue"
      via: "Props for search results, quest items, named enemies and @loot-quest-item / @pull-named-enemy event listeners"
      pattern: "searchResult|questItem|namedEnem|loot-quest-item|pull-named-enemy"
---

<objective>
Publish the SpacetimeDB module with all new tables and reducers, regenerate client bindings, and add minimal client-side integration: search result display in the location panel, quest item loot nodes (with client-side cast timer matching resource gathering pattern), and named enemy pull tiles. Then human-verify the complete quest system end-to-end.
...
8. Wire the `@loot-quest-item` and `@pull-named-enemy` events on the LocationGrid component in App.vue's template. Find the `<LocationGrid ... />` usage and add the event listeners:
```html
<LocationGrid
  ...existing props...
  :quest-items="locationQuestItems"
...
This ensures LocationGrid's `emit('loot-quest-item', qi.id)` and `emit('pull-named-enemy', ne.id)` events are caught by App.vue and dispatched to the correct reducer call handlers.
  </action>
  <verify>
...
- Open the app, verify no console errors related to new table subscriptions
  ...
  <done>Module published with new tables. Bindings regenerated. Client subscribes to QuestItem, NamedEnemy, SearchResult tables. Character-filtered computed properties created. Reducer call handlers wired up. LocationGrid @loot-quest-item and @pull-named-enemy events connected to handler functions in App.vue.</done>
</task>
...
3. Add a QUEST ITEMS section after resources but before enemies, showing discovered but unlooted quest items. **Quest item loot uses a client-side cast timer** following the same pattern as resource gathering: when the player clicks the quest item, show a timed progress bar (3 seconds), and only emit the `loot-quest-item` event when the timer elapses. Look at how resource gathering nodes handle their cast timer in the existing codebase and replicate that pattern. If the existing resource gathering uses a `castTimer` ref or similar mechanism, use the same approach. The server `loot_quest_item` reducer does NOT enforce any timer — this is purely client-side UX:
...
<div v-if="questItems.length > 0">
  ...
  <div :style="styles.gridWrap">
    ...
    >
      <span>{{ qi.name }}</span>
      <span :style="{ fontSize: '0.7rem', opacity: 0.7 }">Click to loot (3s cast)</span>
      <!-- Show cast progress bar if currently casting on this item -->
    </div>
  </div>
</div>
```

Implement `startQuestItemCast(qi)` to start a 3-second client-side timer. When it completes, call `emit('loot-quest-item', qi.id)`. If the character moves or enters combat, cancel the cast. Follow the existing resource gathering cast timer pattern in the codebase.
...
- Click quest item — 3-second cast timer progress bar shows before loot completes
- Named enemies appear when a boss_kill quest is active at that location and the roll succeeds
  </verify>
  <done>LocationGrid displays search results, quest item loot nodes (clickable with client-side cast timer matching resource gathering pattern), and named enemy pull tiles (clickable). Existing LocationGrid sections unchanged. Empty state updated to account for new sections.</done>
</task>
...
- 4 new quest types: kill_loot (item drops from kills), explore (search + loot nodes with cast timer), delivery (hail target NPC), boss_kill (search + named enemy pull)
- Delivery chain: Marla -> Scout Thessa -> Keeper Mordane links all 3 regions
- Client shows search results, quest items (with cast timer), and named enemies in location panel</what-built>
  <how-to-verify>
...
12. Click the quest item to loot it — verify 3-second cast timer shows, then quest completes (and possible aggro)
...
- explore quests reveal items via search, lootable with cast timer and aggro chance
...
- @loot-quest-item and @pull-named-enemy events wired from LocationGrid to App.vue handlers
</verification>
...
- Quest item looting has client-side cast timer (same UX as resource gathering)
