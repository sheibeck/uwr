...
- [ ] **LLM-02**: API key is stored in a private config table, set by admin reducer
- [ ] **LLM-03**: LLM requests are tracked in a status table (pending/processing/completed/error)
- [ ] **LLM-04**: Per-player generation budget limits daily LLM calls
- [ ] **LLM-05**: System prompt uses Anthropic prompt caching for cost reduction
