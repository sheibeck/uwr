...
Last activity: 2026-02-18 - Completed quick task 175: converted 4 insert-only seeding functions to upsert pattern in ensure_items.ts (ensureResourceItemTemplates, ensureGearMaterialItemTemplates, ensureCraftingModifierItemTemplates, ensureRecipeScrollItemTemplates); all 4 now use find-or-insert+update so adding new items or changing fields propagates on plain spacetime publish; module republished without --clear-database and confirmed no seeding errors
