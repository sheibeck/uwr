...
Last activity: 2026-02-19 - Completed quick-198: replace setInterval polling with SpacetimeDB subscription-based version check (AppVersion table, set_app_version admin reducer, window.__client_version, App.vue watch, versionPlugin removed from vite.config.ts). Previous: quick-196 (essence/reagent drop rate tuning), quick-193 (item descriptions + buildItemTooltipData), quick-189 (getModifierMagnitude helper).
