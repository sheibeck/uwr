...
export function getAbilityStatScaling(
  ...
  if (statScaling === 'hybrid') {
    // Hybrid abilities: 60% primary + 40% secondary — detect from character stats
    const { primary, secondary } = detectPrimarySecondary(characterStats);
    if (secondary) {
      const primaryVal = characterStats[primary as keyof typeof characterStats] ?? 0n;
      const secondaryVal = characterStats[secondary as keyof typeof characterStats] ?? 0n;
    ...
    // Fallback: no clear secondary, use primary only
    return characterStats[primary as keyof typeof characterStats] * ABILITY_STAT_SCALING_PER_POINT;
  }
