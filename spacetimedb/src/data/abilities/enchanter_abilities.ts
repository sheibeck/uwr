import type { AbilityMetadata, DamageType } from '../ability_catalog.js';

export const ENCHANTER_ABILITIES: Record<string, AbilityMetadata> = {
  enchanter_mind_fray: {
    name: 'Mind Fray',
    description: 'Assaults the target with psychic energy, dealing magic damage over time.',
    className: 'enchanter',
    resource: 'mana',
    level: 1n,
    power: 3n,
    cooldownSeconds: 6n,
    castSeconds: 1n,
    damageType: 'magic' as DamageType,
    dotPowerSplit: 0.5,
    dotDuration: 3n,
  },
  enchanter_mesmerize: {
    name: 'Mesmerize',
    description: 'Mesmerizes the target with psychic power, stunning them for a short duration.',
    className: 'enchanter',
    resource: 'mana',
    level: 3n,
    power: 2n,
    cooldownSeconds: 15n,
    castSeconds: 0n,
    damageType: 'magic' as DamageType,
    debuffType: 'stun',
    debuffMagnitude: 1n,
    debuffDuration: 4n,
  },
  enchanter_clarity: {
    name: 'Clarity',
    description: 'Blesses an ally with mental clarity, increasing their mana regeneration rate by 1 for 45 minutes.',
    className: 'enchanter',
    resource: 'mana',
    level: 5n,
    power: 3n,
    cooldownSeconds: 5n,
    castSeconds: 2n,
    damageType: 'none' as DamageType,
  },
  enchanter_haste: {
    name: 'Haste',
    description: 'Accelerates the entire party for 30 seconds, increasing auto-attack speed and reducing cast times.',
    className: 'enchanter',
    resource: 'mana',
    level: 7n,
    power: 2n,
    cooldownSeconds: 60n,
    castSeconds: 2n,
    damageType: 'none' as DamageType,
    aoeTargets: 'all_party',
  },
  enchanter_bewilderment: {
    name: 'Bewilderment',
    description: 'Bewilders all enemies, reducing their armor class and hit chance simultaneously.',
    className: 'enchanter',
    resource: 'mana',
    level: 9n,
    power: 3n,
    cooldownSeconds: 15n,
    castSeconds: 2n,
    damageType: 'none' as DamageType,
    aoeTargets: 'all_enemies',
    debuffType: 'ac_bonus',
    debuffMagnitude: -3n,
    debuffDuration: 3n,
  },
  enchanter_charm: {
    name: 'Charm',
    description: 'Charms the current target, spawning a psychic copy of them as your combat pet for the duration.',
    className: 'enchanter',
    resource: 'mana',
    level: 10n,
    power: 0n,
    cooldownSeconds: 300n,
    castSeconds: 2n,
    damageType: 'none' as DamageType,
  },
};
