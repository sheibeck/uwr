export const registerQuestReducers = (deps: any) => {
  ...
  spacetimedb.reducer(
    ...
    (ctx: any, args: any) => {
      ...
      const questItem = ctx.db.questItem.id.find(args.questItemId);
      ...
      // Mark as looted
      ctx.db.questItem.id.update({ ...questItem, looted: true });
      ...
      if (questInstance) {
        ...
        const qt = ctx.db.questTemplate.id.find(questInstance.questTemplateId);
        if (qt) {
      ...
      if (roll < 30n) {
        try {
          ...
          for (const spawn of ctx.db.enemySpawn.by_location.filter(character.locationId)) {
          ...
          if (availableSpawn) {
            ...
            const participants = groupId
              ? [...ctx.db.groupMember.by_group.filter(groupId)]
  ...
  spacetimedb.reducer(
    ...
    (ctx: any, args: any) => {
      ...
      const namedEnemy = ctx.db.namedEnemy.id.find(args.namedEnemyId);
      ...
      const enemyTemplate = ctx.db.enemyTemplate.id.find(namedEnemy.enemyTemplateId);
      if (!enemyTemplate) throw new SenderError('Enemy template not found');

      // Mark the named enemy as not alive
      ctx.db.namedEnemy.id.update({
        ...namedEnemy,
        isAlive: false,
